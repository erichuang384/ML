...
# Nystrom Approximation? For Kernel
...
def _mean_sigma_over_subset(gp, cand_enc, subset=400):
    """Estimate global uncertainty proxy: mean predictive sigma over a subset."""
    n = cand_enc.shape[0]
    m = min(subset, n)
    idx = np.random.choice(n, size=m, replace=False)
    mu, sigma = gp.predict(cand_enc[idx])
    return float(np.mean(sigma))

def _adaptive2_determine_nbo(
    gp, cand_enc, acq_func, X_enc, y_all,
    it, min_batch=2, max_batch=8, subset=400
):
    """
    Determine n_bo by sequential fantasization until global uncertainty proxy
    (mean sigma) falls below a target precision threshold.
    - gp: current fitted GP
    - cand_enc: encoded candidates
    - acq_func: acquisition callable f(mu, sigma) -> acq values
    - X_enc, y_all: current dataset
    - it: current iteration index (0-based)
    Returns: integer n_bo in [min_batch, max_batch]
    """
    # Baseline uncertainty proxy
    base_mean_sigma = _mean_sigma_over_subset(gp, cand_enc, subset=subset)

    # Iteration-specific precision target (gradually tightens; bounded)
    # Target decays gently so early iterations prefer larger batches, aligning with paper insight  ^1^  ^2^ .
    decay = 0.85 ** (it + 1)
    target = max(0.15 * base_mean_sigma, base_mean_sigma * decay)

    # Temporary GP for fantasization
    gp_temp = GP(gp.ls, gp.sf, gp.noise)
    gp_temp.fit(X_enc, y_all)

    # Sequentially fantasize picks using a simple believer update (kriging believer)
    n_bo = 0
    mean_sigma = base_mean_sigma
    # Safety cap on tried candidates when computing acq
    tries = 0
    while mean_sigma > target and n_bo < max_batch:
        # Acquisition on full pool
        mu, sigma = gp_temp.predict(cand_enc)
        acq_vals = acq_func(mu, sigma)
        # Exclude already fantasized indices by setting -inf
        # We'll track previously chosen by selecting argmax on each step
        # Note: This loop is only for determining n_bo; actual selection later uses your chosen batch_method
        best_idx = int(np.argmax(acq_vals))
        # Fantasize: add selected point with believer label (mu at that point)
        y_fake = float(mu[best_idx])
        gp_temp.fit(
            np.vstack([gp_temp.X, cand_enc[best_idx]]),
            np.append(y_all, y_fake)
        )
        n_bo += 1
        mean_sigma = _mean_sigma_over_subset(gp_temp, cand_enc, subset=subset)
        tries += 1
        if tries > 3 * max_batch:
            break

    return max(min_batch, n_bo)
...
def run_bo(acq_kind, batch_method, batch_size, iters=15, seed=None):
    ...
    for it in range(iters):
        if batch_size == "adaptive":
            current_batch = max(2, 5 - (it // 3))
        elif batch_size == "adaptive2": # https://arxiv.org/pdf/2306.05843
            # Fit GP (we already do this below; do it here to get n_bo)
            gp = GP(ls=0.5*(1 + 0.8*min(stagnation/5,1)))
            gp.fit(X_enc, y_all)
            acq = make_acq(acq_kind, best_seen, stagnation)

            # Determine number of BO picks adaptively via sequential fantasization
            n_bo_adapt = _adaptive2_determine_nbo(
                gp, cand_enc, acq, X_enc, y_all, it,
                min_batch=2, max_batch=8, subset=400
            )
            # Form total batch size with 30% random, like your default
            current_batch = n_bo_adapt
        else:
    ...
    # ...evaluation, logging, stagnation update unchanged...
...
if __name__ == "__main__":
    ...
    iterations = 15
    n_repeats = 1   # Suggest at least 3-5 for good stats
