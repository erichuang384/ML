...
def matern52_kernel(X1, X2, length_scale=1.0, sigma_f=1.0):
    """
    Matérn 5/2 kernel (ARD-supported).
    k(x, x') = sigma_f^2 * exp(-sqrt(5)*r) * (1 + sqrt(5)*r + 5/3*r^2)
    with r^2 = sum_j ((x_j - x'_j)^2 / ell_j^2).
    """
    X1 = np.asarray(X1, dtype=float)
    X2 = np.asarray(X2, dtype=float)

    ell = np.asarray(length_scale, dtype=float)
    if ell.ndim == 0:
        ell = np.full(X1.shape[1], float(ell))
    assert ell.shape[0] == X1.shape[1] == X2.shape[1], "length_scale must match feature dimension"
    ...
    sqdist = (
        np.sum(X1s**2, axis=1).reshape(-1, 1)
        + np.sum(X2s**2, axis=1)
        - 2.0 * (X1s @ X2s.T)
    )
    ...
    term = (1.0 + sqrt5_r + (5.0/3.0) * (r**2))
    K = (sigma_f**2) * np.exp(-sqrt5_r) * term
    return K
...
class BO:
    ...
    def _select_batch(self, batch_size):
        pool_indices_global = [i for i, avail in enumerate(self.available) if avail]
        if len(pool_indices_global) == 0:
            return [], np.empty((0, self.X_enc.shape[1])), []

        # Local pool aligned with local indices [0..len-1]
        X_pool_enc = self.X_search_enc[pool_indices_global].copy()
        ...
        if self.strategy == 'kriging_believer':
            return self._select_batch_believer(
                X_pool_enc, pool_indices_global, X_tmp, Y_tmp, mode='kb'
            )
        elif self.strategy == 'pessimistic_believer':
            return self._select_batch_believer(
                X_pool_enc, pool_indices_global, X_tmp, Y_tmp, mode='pb'
            )
        elif self.strategy == 'constant_liar':
            return self._select_batch_constant_liar(
                X_pool_enc, pool_indices_global, X_tmp, Y_tmp
            )
    ...
    # ---------- Helper strategies ----------
    def _make_temp_gp(self):
        # Temporary GP mirrors main GP hyperparameters
        return GaussianProcess(
            length_scale=self.gp.l,
            sigma_f=self.gp.sigma_f,
            noise=self.gp.noise,
            mean_func=self.gp.mean_func
        )

    
    def _select_batch_believer(self, X_pool_enc, pool_idx_global, X_tmp, Y_tmp, mode='kb'):
        selected_raw = []; selected_enc = []; selected_indices_global = []
        n_select = min(self.batch, len(pool_idx_global))  # how many to pick this round
        for _ in range(n_select):
            gp_tmp = self._make_temp_gp()
            gp_tmp.fit(X_tmp, Y_tmp)

            Y_best_tmp = np.max(Y_tmp) if self.maximize else np.min(Y_tmp)
            # Evaluate acquisition on the local pool directly
            ...
            choice_global = pool_idx_global[loc]

            x_best_enc = X_pool_enc[loc]
            x_best_raw = self.X_search_raw[choice_global]
            mu_sel, var_sel = gp_tmp.predict([x_best_enc])
            mu_sel = float(mu_sel[0])
            ...
            selected_indices_global.append(choice_global)

            # Remove selected from local pool & global list to keep alignment
            X_pool_enc = np.delete(X_pool_enc, loc, axis=0)
            pool_idx_global.pop(loc)

        return selected_raw, np.array(selected_enc, dtype=float), selected_indices_global

    def _select_batch_constant_liar(self, X_pool_enc, pool_idx_global, X_tmp, Y_tmp):
        selected_raw = []; selected_enc = []; selected_indices_global = []
        for _ in range(min(self.batch, len(pool_idx_global))):
            gp_tmp = self._make_temp_gp()
            gp_tmp.fit(X_tmp, Y_tmp)
    ...
    def _select_batch_local_penalization(self, X_pool_enc, pool_idx_global, X_tmp, Y_tmp):
        selected_raw = []; selected_enc = []; selected_indices_global = []
        centers = []; radii = []
        ...
        for _ in range(min(self.batch, len(pool_idx_global))):
            ...
            Y_best_tmp = np.max(Y_tmp) if self.maximize else np.min(Y_tmp)
            X_eval = X_pool_enc.copy()  # evaluate on local pool directly
            acq_vals, goal = compute_acquisition(
                X_eval, gp_tmp, Y_best=Y_best_tmp, kind=self.acquisition,
                maximize=self.maximize, beta=self.beta, xi=self.xi
            )

            # Apply local penalization around already selected centers
            if len(centers) > 0:
                D2 = np.array([np.sum((X_eval - c)**2, axis=1) for c in centers])
    ...
    def _select_batch_thompson_sampling(self, X_pool_enc, pool_idx_global, X_tmp, Y_tmp):
        selected_raw = []; selected_enc = []; selected_indices_global = []
        ...
        for _ in range(min(self.batch, len(pool_idx_global))):
            ...
            if len(pool_idx_global) > self.ts_subset:
                sub_idx_local = np.random.choice(len(pool_idx_global), size=self.ts_subset, replace=False)
            else:
                sub_idx_local = np.arange(len(pool_idx_global))
            X_eval = X_pool_enc[sub_idx_local]

            mu_eval, _ = gp_tmp.predict(X_eval)
            C_eval = gp_tmp.posterior_cov(X_eval)
            ...
            loc = int(sub_idx_local[loc_sub])  # map back to local pool index
            choice_global = pool_idx_global[loc]
            ...
            # Remove selected from local + global pools
            X_pool_enc = np.delete(X_pool_enc, loc, axis=0)
            pool_idx_global.pop(loc)
